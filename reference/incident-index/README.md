...
| INC-016: Webhook returns 200 OK but business state is wrong | Stripe + Supabase | Event-ID dedup passes, but side effects are not independently idempotent. The failure is two-sided: marking the event processed before its side effects commit produces duplicates, while a first attempt that fails after recording the event ID makes retries skip it as already seen — silently dropping the event. | Track processing state (`received` → `processing` → `processed` / `failed`), not just "seen"; give each side effect its own idempotency key; return 200 only after side effects commit and non-2xx for unfinished work so Stripe retries; put a lease and attempts cap on `processing`; reconcile expired or missed events from the Stripe API. | [Stripe webhook state machine](../diagrams/stripe-webhook-state-machine.md), [Stripe webhook idempotency template](../templates/stripe-webhook-idempotency-template.sql), [Stripe webhook test plan](../playbooks/stripe-webhook-test-plan.md) |
